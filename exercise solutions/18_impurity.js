...
// Wrap it around sortStudentsBy name to contain the impurity
function getStudentsByName(students) {
  // make a local copy of students
  students = students.slice(); // shallow copy of student
  return sortStbudentsByName(); // will produce a side effect only on the local students variable

  function sortStbudentsByName() {
...
function getStudentsByID(currentStudents) {
  ...
  var originalStudents = [...students]; // Shallow copy first level copied and deeper levels are referenced for deep copy can use Object.assign with map
  students = currentStudents.slice();
  // modify and store the new state
